 ...
 * 
 *   This file featuers an optimised version of the NeuralNetwork using OpenMP.
 *   To allow some interesting, finer-grained parallelisations, the sample loop
 *   and layer loop have been swapped. The program can now be through of as a
 *   pipeline, where each loop is a pipeline.
**/
...
void nn_train(neural_net* nn, size_t n_samples, array* inputs[n_samples], array* expected[n_samples], double learning_rate, size_t n_iterations, double (*act)(double), double (*dydx_act)(double)) {
    ...
    for (size_t i = 0; i < n_iterations; i++) {
        // Loop through all layers in the network for the forward pass
        for (size_t l = 0; l < nn->n_layers; l++) {
            // Note the bias and weight for this layer
            array* bias = nn->biases[l - 1];
            matrix* weight = nn->weights[l - 1];
            ...
            for (size_t s = 0; s < n_samples; s++) {
                ...
                for (size_t n = 0; n < nn->nodes_per_layer[l]; n++) {
                    ...
                    double z = bias->d[n];
                    for (size_t prev_n = 0; prev_n < nn->nodes_per_layer[l - 1]; prev_n++) {
                        z += layer_outputs[s][l - 1]->d[prev_n] * INDEX(weight, prev_n, n);
                    }
