...
void nn_train(neural_net* nn, size_t n_samples, double** inputs, double** expected, double learning_rate, size_t n_iterations) {
        ...
        #pragma omp parallel
        {
            ...
            for (size_t s = 0; s < n_samples; s++) {
                ...
                if (TID == 0) {
                    ...
                    t_crit_out += TIMEVAL_TO_MS(s_crit, e_crit);
                    #endif
                }
                ...
                for (size_t l = n_layers - 2; l > 0; l--) {
                    ...
                    #ifndef BENCHMARK
                    gettimeofday(&e_crit, NULL);
                    t_crit_hid[TID] += TIMEVAL_TO_MS(s_crit, e_crit);
                    #endif
    ...
    #else
    double total_crit_out = t_crit_out;
    double total_crit_hid = 0;
    for (size_t t = 0; t < n_threads; t++) {
        total_crit_hid += t_crit_hid [t];
    }
