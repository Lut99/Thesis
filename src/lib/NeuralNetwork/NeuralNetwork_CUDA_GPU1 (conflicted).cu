...
/* Kernel that computes the forward pass for a single layer. */
__global__ void FwdPass(neural_net* nn, )
...
void nn_train(neural_net* nn, size_t n_samples, array* inputs[n_samples], array* expected[n_samples], double learning_rate, size_t n_iterations, double (*act)(double), double (*dydx_act)(double)) {
    ...
    double** biases = (double**) malloc(sizeof(double*) * (nn->n_layers - 1));
    for (size_t l = 0; l < nn->n_layers - 1; l++) {
    ...
    // Copy the biases array to the GPU as well
    double** biases_gpu;
    cudaMemcpy((void*) biases_gpu, (void*) biases, sizeof(double) * (nn->n_layers - 1), cudaMemcpyHostToDevice);
    ...
    double** weights = (double**) malloc(sizeof(double*) * (nn->n_layers - 1));
    size_t* weights_pitches = (size_t*) malloc(sizeof(size_t) * (nn->n_layers - 1));
    for (size_t l = 0; l < nn->n_layers - 1; l++) {
        cudaMallocPitch((void**) weights + l, weights_pitches + l, sizeof(double) * nn->nodes_per_layer[l], nn->nodes_per_layer[l + 1]);
    }

    // Create a list of double arrays that is used to store intermediate outputs.
    //   Note that the arrays themselves are already allocated on the GPU for
    //   compatibility. (1797 x 2 iterations)
    double** layer_outputs = (double**) malloc(sizeof(double*) * n_samples * nn->n_layers);
    for (size_t s = 0; s < n_samples; s++) {
        for (size_t l = 0; l < nn->n_layers; l++) {
            cudaMalloc((void**) &layer_outputs[s * nn->n_layers + l], sizeof(double) * nn->nodes_per_layer[l]);
        }

        // Copy the input over to this list
        cudaMemcpy(layer_outputs[s * nn->n_layers], inputs[s]->d, sizeof(double) * nn->nodes_per_layer[0], cudaMemcpyHostToDevice);
    }
    // Copy this final list to a pitched GPU array so that the GPU can actually use it
    double** layer_outputs_gpu;
    size_t layer_outputs_pitch;
    cudaMallocPitch((void**) &layer_outputs_gpu, &layer_outputs_pitch, sizeof(double) * nn->n_layers, nn->n_layers, n_samples);
    cudaMemcpy2D((void*) layer_outputs_gpu, layer_outputs_pitch, (void*) layer_outputs, sizeof(double) * nn->n_layers, sizeof(double) * nn->n_layers, n_samples, cudaMemcpyHostToDevice);
