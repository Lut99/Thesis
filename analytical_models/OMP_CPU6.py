...
def t_bck_out(m_params, n_threads, L, P):
    return sequential.t_bo_delta(m_params[n_threads][2], m_params[n_threads][3], P[L - 1]) + \
           sequential.t_b_updbias(m_params[1][2], m_params[1][3], P[L - 1]) + \
           sequential.t_b_updweights(m_params[1][2], m_params[1][3], P[L - 2], P[L - 1])
...
def t_bh_delta(m_params, n_threads, p, p1):
    return p * (sequential.t_bh_sum(m_params[n_threads][2], m_params[n_threads][3], p1) + \
                sequential.t_bh_comp(m_params[n_threads][0], m_params[n_threads][1]))
